     ...
     <main className="min-h-screen grid plaice-items-center justify-center ">
        <section className="max-w-4xl mx-auto px-4 py-14 sm:px-6 sm:py-20 md:py-28">
          ...
          {artistData.map((item, index) => (
            <section className="relative z-50 -top-8 flex bg-secondary rounded-2xl p-6 mb-3" key={index}>
                    <div className="flex gap-6">
                      {/* AVATAR*/}
                      <img className="rounded-full" src={item.avatar} width={150} height={80} alt="Avatar of Kam" />
                        <div>
                          {/* PROJECT NAME */}
                          <h1 className="font-bold">{item.project}</h1>
                          {/* ARTIST */}
                          <p className="font-bold pb-5">{item.artist}</p>
                          {/* FINGERPRINT ICON */}
                ...
                <p>
                  Tempor irure est est eiusmod incididunt <br /> mollit deserunt occaecat non mollit.
                </p>
                {/* ALBUM COVER*/}
                <img className="absolute top-0 right-0 h-full rounded-r-2xl" src={item.cover} alt="Album cover" />
            </section>
          ))}
        <section>
        {/* MUSIC PLAYER */}
